...
public class SMat implements Matrix {
    ...
    public static void main(String[] args) throws IOException {
        DMat d1 = new DMat("1.txt");
        DMat d2 = new DMat("3.txt");
        SMat s1 = new SMat("31.txt");
        SMat s2 = new SMat("34.txt");
        ...
        if (d1.arr.length == d2.arr.length) {
            ...
            try{
            DMat ddd = (DMat) d1.pmul(d2);
            ddd.saveToFile("pmulDDResult.txt");
            }catch (Exception E){}
            if (s1.sizeOfMatrix == s2.sizeOfMatrix) {
                SMat ss = (SMat) s1.mul(s2);
                ss.saveToFile("mulSSResult.txt");
                try{
                SMat ddd = (SMat) s1.pmulSS(s2);
                ddd.saveToFile("pmulSSResult.txt");
                }catch (Exception E){}
